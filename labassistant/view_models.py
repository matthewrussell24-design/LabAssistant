...
def parse_uploaded_file(uploaded_file) -> ParsedSample:
...
def sample_from_measurement(measurement: Measurement) -> ParsedSample:
...
def sample_status(sample: ParsedSample) -> str:
    return status_from_warnings(sample.warnings)


def build_metrics_table(samples: list[ParsedSample]) -> pd.DataFrame:
    rows = []
...
def build_angle_table(samples: list[ParsedSample]) -> pd.DataFrame:
    """One row per sample per scattering angle for dual-angle DLS runs.

    Empty (no columns beyond the schema) when no sample reports multiple angles,
    so callers can simply check ``.empty`` before rendering a per-angle view.
    """
