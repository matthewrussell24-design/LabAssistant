...
<main class="app">
  ...
  <div class="layout">
    <section class="card workspace"><h2>Workspace</h2><div class="muted">Start with an experiment, then bring in evidence.</div><div class="actions">
      ...
      <button class="action" disabled><strong>Open Existing Experiment</strong><span>Persisted history planned</span></button>
    ...
    <section class="card history"><h2>History</h2><div class="muted">Recent work from this desktop session.</div><div class="history-list" id="history"></div><div class="history-foot">Persisted restore will be added through an application contract.</div></section>
  </div>
</main>
<script>
...
function history(){const el=document.getElementById('history');if(!state.history.length){el.innerHTML='<div class="muted" style="margin-top:14px">No experiments yet.<br>Imported analyses will appear here.</div>';return;}el.innerHTML=state.history.map((x,i)=>`<div class="history-item" onclick="restore(${i})"><strong>${esc(x.experiment.label)}</strong><span>Just now &nbsp;·&nbsp; ${x.experiment.measurement_count} measurement${x.experiment.measurement_count===1?'':'s'}</span></div>`).join('');}
